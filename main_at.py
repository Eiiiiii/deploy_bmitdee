...
if choice == "Home Page":
    ...
    if berat_badan and tinggi_badan:
        ...
        st.write("BMI: {:.2f}".format(body_mass_index))
        if body_mass_index <18.5:
            st.write('Berat Badan anda Kurang!')
        elif 18.5 <= body_mass_index <= 22.9 : 
            st.write('Berat Badan anda Normal!')
        elif 23 <= body_mass_index <= 24.9 : 
    ...
    if berat_badan_bmr and tinggi_badan_bmr and usia:
        if gender == 'Pria':
            ...
            st.write("BMI: {:.2f}".format(bmr))
            st.write('Anda dapat tidur seharian dengan "sehat" jika anda makan setidaknya 2x sehari 😄')
        elif gender == 'Wanita':
