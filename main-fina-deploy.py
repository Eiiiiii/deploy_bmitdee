...
options = ["BMI Calculator","BMR Calculator" , "TDEE Calculator" , "Kalori list" , "Coba"]
...
if choice == "BMI Calculator":
    ...
    st.markdown("---")
    st.write("""
    Web ini dibuat untuk melaksanakan project LPK kelompok 11 yang beranggotakan\n
    -Deri Hamsa\n
    -Dimas Bagus Uditya\n
    -Sabrina Annisafitri\n
             """)
    
    st.subheader('Input Data yang dibutuhkan:')
    st.markdown("---")
...
#         st.image("")
        for index, row in df_karbo.iterrows():
            ...
            if st.checkbox('Tambah Makanan/Minuman ini', key=f'tambah1_{index}_k'):
...
#         st.image("")
        ...
        for index, row in df_protein.iterrows():
            ...
            if st.checkbox('Tambah Makanan/Minuman ini', key=f'tambah1_{index}_p'):
                # Tambahkan makanan/minuman yang dipilih beserta kalori ke dictionary
...
#         st.image("")
        ...
        for index, row in df_sayur.iterrows():
            st.write('\n',row['Nama_s'], row['kalori_s'])
            if st.checkbox('Tambah Makanan/Minuman ini', key=f'tambah1_{index}_s'):
                # Tambahkan makanan/minuman yang dipilih beserta kalori ke dictionary
                selected_items[row['Nama_s']] = row['kalori_s']
